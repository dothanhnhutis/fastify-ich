import { GetWarehousesByPackagingIdQueryType } from "@/modules/v1/packagings/packaging.schema";
...
export default class PackagingStockRepo {
  ...
  async findByPackagingId(
  ...
  ): Promise<{ warehouses: WarehouseStock[]; metadata: Metadata }> {
    const newTable = `
    WITH
    packaging_stocks AS (
        SELECT
    ...
    try {
      ...
      const { rows: warehouses }: QueryResult<WarehouseStock> =
        await this.fastify.query<WarehouseStock>(queryConfig);
